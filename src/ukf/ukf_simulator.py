...
if __name__ == "__main__":
    ...
    print("Mean position error:", np.mean(pos_error), "m")
    print("Max position error:", np.max(pos_error), "m")
    print("Std of position error:", np.std(pos_error), "m")

    # Compare estimated vs true velocities
    vel_error = np.linalg.norm(true_states[:, 3:6] - estimates[:, 3:6], axis=1)
    print("Mean velocity error:", np.mean(vel_error), "m/s")
    print("Max velocity error:", np.max(vel_error), "m/s")
    print("Std of velocity error:", np.std(vel_error), "m/s")
